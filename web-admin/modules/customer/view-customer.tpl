<div class="row-fluid">
    <div class="span12">
        <h3 class="heading">CUSTOMERS <img src="{$baseurl}/assets/img/loader.gif" id="loader" style="display:" /></h3>
            ...
            <form name="frmpage" id="frmpage"  method="post" action="javascript:return false;" >
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6" align="right">
                    	...
                    	<a class="btn" title="Export to Excel" href="javascript:exportdata();">
                    		<i class="splashy-document_a4_download"></i>
                    	</a>
                    </div>
                </div>
...
function hapusdata(id) {
	...
	$.ajax({
	  type : 'POST',
	  url : '{$baseurl}/customer/query-delete',
	  data : {
	    'id' : id
	  },
	  beforeSend : function(){
...
function tambahdata() {
	ajaxpage('{$baseurl}/customer/add', 'Add Customer');
}

function editdata(id) {
	ajaxpage('{$baseurl}/customer/edit/'+id, 'Edit Customer');
}
function exportdata(){
  var namafile = prompt('Masukkan nama file : ');
  if(namafile != null){
    window.location = "{$baseurl}/customer/export?" + $('#frmpage').serialize() + "&namafile=" + namafile;
  }
}
</script>
