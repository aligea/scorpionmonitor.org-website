<div class="row-fluid">
    <div class="span12">
        <h3 class="heading">GROUP USERS <img src="{$baseurl}/assets/img/loader.gif" id="loader" style="display:" /></h3>
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6" align="right">
                        <select name="pilihfilter" class="span3" id="pilihfilter">
                            ...
                            <option value="A.grupuser">Grup user</option>
                        </select>
		...
		success : function(r) {	
			...
			if (json.databody != null) {
				$.each(json.databody, function(m, item) {
					...
					field += '<td><a class="btn btn-mini" title="Lihat user di grup ini" href="{$baseurl}/user?grup='+item.grupuser+'"> '+item.jumlahuser+' user </a><a class="btn btn-mini" title="Otorisasikan grupuser ini" href="javascript:otorisasi('+item.id+')"><i class="icon icon-user"></i></a>';
...
function hapusdata(id) {
	...
	$.ajax({
	  type : 'POST',
	  url : '{$baseurl}/groupuser/query-delete',
	  data : {
	    'id' : id
	  },
	  beforeSend : function(){
...
function otorisasi(idgrupuser){
	ajaxpage('{$baseurl}/auth?id='+idgrupuser, 'Otorisasi Grup user - Module');
}
function tambahdata() {
	ajaxpage('{$baseurl}/groupuser/add', 'Add Group User');
}

function editdata(id) {
	ajaxpage('{$baseurl}/groupuser/edit/'+id, 'Edit Group User');
}
