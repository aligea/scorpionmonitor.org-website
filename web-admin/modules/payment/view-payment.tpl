<div class="row-fluid">
    <div class="span12">
        <h3 class="heading">PAYMENT METHODS <img src="{$baseurl}/assets/img/loader.gif" id="loader" style="display:" /></h3>
            ...
            <form name="frmpage" id="frmpage"  method="post" action="javascript:return false;" >
            	<div class="row-fluid">
                  <div class="span12">
                        ...
                        {foreach from=$datajenis item=data}
		...
		success : function(r) {	
			...
			if (json.databody != null) {
				$.each(json.databody, function(m, item) {
					...
					field += '<td>'+item.jenis+'</td>';
					field += '<td>'+item.nama+'</td>';
					field += '<td>'+item.rekening+'</td>';
...
function hapusdata(id) {
	...
	$.ajax({
	  type : 'POST',
	  url : '{$baseurl}/payment/query-delete',
	  data : {
	    'id' : id
	  },
	  beforeSend : function(){
...
function tambahdata() {
	ajaxpage('{$baseurl}/payment/add', 'Add Payment Methode');
}

function editdata(id) {
	ajaxpage('{$baseurl}/payment/edit/'+id, 'Edit Payment Methode');
}
