...
<input type="hidden" id="id" name="id" value="{$data.id}" />
  <fieldset class="well">
  <p class="f_legend">Edit Payment Methode</p>
  <div class="row-fluid">
    <div class="span6">
      ...
      <div class="control-group">
        ...
        <div class="controls">
          ...
          {foreach from=$datajenis item=obj}
            <option value="{$obj.value}" {if $obj.value == $data.jenis} selected="selected" {/if}>{$obj.text}</option>
          {/foreach}  
          </select>
        </div>
      </div>
      ...
      <div class="control-group">
        ...
        <div class="controls">
          <input type="text" name="rekening" id="rekening" placeholder="" class="validate[required]" value="{$data.rekening}" />
        </div>
      </div>
      ...
      <div class="control-group">
        ...
        <div class="controls">
          <input type="text" name="atas_nama" id="atas_nama" placeholder="" class="validate[required]" value="{$data.atas_nama}" />
        </div>
      </div>
    ...
    <div class="span6">
      <div class="control-group">
        ...
        <div class="controls">
          <input type="text" class="form-control validate[]" id="logo" placeholder="url gambar" name="logo" onchange="document.getElementById('view-logo').src = this.value;" value="{str_replace('[baseurlroot]', $baseurlroot, $data.logo)}">
          <a class="btn btn-small" href="javascript:browseImage('logo')" title="Browse Image"><i class="splashy-image_cultured"></i></a>
          <img id="view-logo" class="img-responsive img-thumbnail thumbnail" style="width:150px;height:120px;" src="{str_replace('[baseurlroot]', $baseurlroot, $data.logo)}" />
        </div>
      </div>
...
function submitForm(formElm){
  ...
  $.ajax({
    url : '{$baseurl}/payment/query-update',	
    type:'POST',
