  ...
  <div class="row">
    ...
    <div class="col-md-6">
      ...
      <select name="pilihfilter" class="form-control" id="pilihfilter">
        ...
        <option value="alias">Module Page</option>
...
function izydata(id) {
	...
	$.ajax({
		...
		success : function(r) {	
			...
			if (json.databody == "") {
				$("#tabeldata tbody").append('<tr><td colspan="2" style="text-align:center;">No available data.</td></tr>');
			}
...
function tambahdata() {
	ajaxpage('{$baseurl}/page/add', 'Add Page');
}

function editdata(id) {
	ajaxpage('{$baseurl}/page/edit/'+id, 'Edit Page');
}
...
function deleteSelectedItems(){
	...
	$.ajax({
	  type : 'POST',
	  url : '{$baseurl}/page/query-delete-selected-items',
	  data : {
	    'dataid' : itemToDelete
	  },
	  beforeSend : function(){
...
function editSelectedItems(){
	...
	$.each(items, function(key, value){
	  ...
	  window.open('{$baseurl}/page/edit/'+items[key].value);
	});
}
