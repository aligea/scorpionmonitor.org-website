<div class="row-fluid">
    <div class="span12">
        <h3 class="heading">USERS <img src="{$baseurl}/assets/img/loader.gif" id="loader" style="display:" /></h3>
            ...
            <form name="frmpage" id="frmpage"  method="post" action="javascript:return false;" >
            	<div class="row-fluid">
                  <div class="span12">
                        ...
                        {foreach from=$datagrupuser item=data}
                          <option value="{$data.grupuser}" {if $data.grupuser == $selectedgrup} selected="selected" {/if} >{$data.grupuser}</option>
                        {/foreach}  
                        </select>
                  </div>
                </div>
...
function tambahdata() {
	ajaxpage('{$baseurl}/user/add', 'Add User');
}

function editdata(id) {
	ajaxpage('{$baseurl}/user/edit/'+id, 'Edit User');
}
