<div class="row-fluid">
    <div class="span12">
        <h3 class="heading">PRODUCTS <img src="{$baseurl}/assets/img/loader.gif" id="loader" style="display:" /></h3>
            ...
            <form name="frmpage" id="frmpage"  method="post" action="javascript:return false;" >
            	<div class="row-fluid">
                  <div class="span12">
                        ...
                        {foreach from=$datacategory item=data}
                          <option value="{$data.id}">{$data.parentcategory}{$data.name}</option>
                        {/foreach}  
                        </select>
                  </div>
                </div>
                <div class="row-fluid">
                    ...
                    <div class="span6" align="right">
                        <select name="pilihfilter" class="span3" id="pilihfilter">
                            ...
                            <option value="A.name">Nama produk</option>
		...
		success : function(r) {	
			...
			if (json.databody != null) {
				$.each(json.databody, function(m, item) {
					...
					field += '<td>'+item.categoryname+'</td>';
					field += '<td>'+item.name+'</td>';
					field += '<td style="text-align:right;">'+numeral(item.price).format('0,0')+'</td>';
...
function hapusdata(id) {
	...
	$.ajax({
	  type : 'POST',
	  url : '{$baseurl}/product/query-delete',
	  data : {
	    'id' : id
	  },
	  beforeSend : function(){
...
function tambahdata() {
	ajaxpage('{$baseurl}/product/add', 'Add Product');
}

function editdata(id) {
	ajaxpage('{$baseurl}/product/edit/'+id, 'Edit Product');
}
