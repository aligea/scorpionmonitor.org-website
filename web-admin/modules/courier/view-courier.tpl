<div class="row-fluid">
    <div class="span12">
        <h3 class="heading">COURIERS <img src="{$baseurl}/assets/img/loader.gif" id="loader" style="display:" /></h3>
		...
		success : function(r) {	
			...
			if (json.databody != null) {
				$.each(json.databody, function(m, item) {
					...
					field += '<td><a href="#">'+item.jumlahtarif+' data tarif</a></td>';
...
function hapusdata(id) {
	...
	$.ajax({
	  type : 'POST',
	  url : '{$baseurl}/courier/query-delete',
	  data : {
	    'id' : id
	  },
	  beforeSend : function(){
...
function tambahdata() {
	ajaxpage('{$baseurl}/courier/add', 'Add Courier');
}

function editdata(id) {
	ajaxpage('{$baseurl}/courier/edit/'+id, 'Edit Courier');
}
