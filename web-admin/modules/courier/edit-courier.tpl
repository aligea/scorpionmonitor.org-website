...
<input type="hidden" id="id" name="id" value="{$data.id}" />
  <fieldset class="well">
  <p class="f_legend">Edit Courier</p>
  <div class="row-fluid">
    <div class="span6">
      ...
      <div class="control-group">
        ...
        <div class="controls">
          <select name="status" id="status">
            <option value="Y" {if $data.status == 'Y'} selected="selected" {/if}>Aktif</option>
            <option value="N" {if $data.status == 'N'} selected="selected" {/if}>Nonaktif</option>            
          </select>
        </div>
      </div>
      ...
      <div class="control-group">
        ...
        <div class="controls">
          <textarea name="keterangan" id="keterangan" class="">{$data.keterangan}</textarea>
        </div>
      </div>
...
function submitForm(formElm){
  ...
  $.ajax({
    url : '{$baseurl}/courier/query-update',	
    type:'POST',
